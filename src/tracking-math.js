...
export function bodySignals(world, image) {
  if (![11,12,13,14,15,16,23,24].every(i=>visible(world?.[i]))) return null;
  const right=unit(subtract(world[12],world[11]));
  const down=unit(subtract(midpoint(world[23],world[24]),midpoint(world[11],world[12])));
  if(!right || !down) return null;
  const forward=unit(cross(right,down));
  if(!forward) return null;
  const signals={};
  for(const [side,s,e,w,sign] of [['left',11,13,15,-1],['right',12,14,16,1]]) {
    const upper=unit(subtract(world[e],world[s])), lower=unit(subtract(world[w],world[e]));
    if(!upper || !lower) return null;
    signals[`${side}Elbow`]=Math.acos(clamp(dot(upper,lower),-1,1))/(Math.PI/2);
