...
For Pages, the build includes gzip camera binaries. Supporting browsers download and decompress the WASM and model concurrently in the worker; the original files remain as a compatibility fallback. Initial setup allows two minutes and can be cancelled immediately. This avoids the old 30-second failure on slow first visits.
