...
The original timeline's `PlayAudio` uses Unity `AudioSource.Play` with 44.1 kHz sample buffers on the phone. Its `TriggerVoice`, `WordDetection`, and `SpectrumMicrophone` code also runs recognition on the phone and triggers timeline nodes; it does not send speech to the robot. No arbitrary audio-upload or robot text-to-speech command was found. Robot speech is limited to firmware content invoked with command 19. The robot-reported bank exposes INTRODUCE, HI FIVE, JOKE, SYSTEM CHECK, USER NAME, ROBOT NAME, TELL TIME, and SHAKE HANDS alongside its movement routines. Microphone checks remain manual; no raw level stream is implemented.
