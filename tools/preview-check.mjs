...
try{
 ...
 await page.locator('#disconnect').click();await page.evaluate(async()=>{const {RobotPreview}=await import('./src/robot-preview.js');const {defaultCalibration,MEASURED_NEUTRAL}=await import('./src/joints.js');const host=document.createElement('div');host.style='width:300px;height:400px';document.body.append(host);const preview=new RobotPreview(host,document.createElement('p'));const config=defaultCalibration(),pose=[...MEASURED_NEUTRAL];pose[1]=config[1].max;preview.setPose(pose,config);window.previewShortSide=preview.targets[1];pose[1]=config[1].min;preview.setPose(pose,config);window.previewLongSide=preview.targets[1];const reversed=config.map(c=>({...c,reversed:!c.reversed}));preview.setPose(pose,reversed);window.previewReversed=preview.targets[1];preview.setSignals({rightElbow:1,headTurn:.7,headTilt:-.5},config,.8);window.virtualTestPose=JSON.parse(host.dataset.pose);preview.dispose();host.remove();});
