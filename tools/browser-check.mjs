...
try {
 ...
 assert.equal(await page.locator('#mirrorStart').isDisabled(),true);
 ...
 await page.waitForFunction(()=>document.querySelector('#cameraState').textContent.includes('Tracking lost')||document.querySelector('#cameraState').textContent.includes('Body tracked')||document.querySelector('#notice').textContent.includes('Error'),{},{timeout:45000});
 const cameraState=await page.locator('#cameraState').textContent();assert.match(cameraState,/Tracking lost|Body tracked/);
